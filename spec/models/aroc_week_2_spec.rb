...
# ----------------------
  ...
  it '9. finds orders for a user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders_of_user_3 = Order.where("user_id % 3 = 0")
    # ------------------------------------------------------------
  ...
  it '12. should return all items except items 2, 5 and 6' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    string_ids = items_not_included.map {|item| item.id }.join(', ')
    items = Item.where("id not in (#{string_ids})")
    # ------------------------------------------------------------
  ...
  it "13. groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    grouped_items = Item.joins("left join order_items on items.id = order_items.item_id")
                        .where("order_items.order_id = #{@order_3.id}")
                        .order("items.name")
    # ------------------------------------------------------------
  ...
  it '14. plucks all values from one column' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    names = Item.select(:name).map(&:name)
    # ------------------------------------------------------------
  ...
  it '15. gets all item names associated with all orders' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    names =   Item.joins("right join order_items on items.id = order_items.item_id")
                  .map(&:name)
    # ------------------------------------------------------------
